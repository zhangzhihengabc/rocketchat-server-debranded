...
import type { IEmailInbox, IEmoji, IInquiry, IInstanceStatus, IIntegration, IIntegrationHistory, ILivechatDepartmentAgents, ILoginServiceConfiguration, IMessage, INotificationDesktop, IPbxEvent, IRole, IRoom, ISetting, ISocketConnection, ISubscription, IUser, IUserStatus, IInvite, IWebdavAccount, ICustomSound, VoipEventDataSignature, UserStatus, ILivechatPriority, VideoConference, ICalendarNotification, AtLeast, ILivechatInquiryRecord, ILivechatAgent, IBanner, ILivechatVisitor, UiKit, LicenseLimitKind } from '@rocket.chat/core-typings';
import type { AutoUpdateRecord } from './types/IMeteor';
type ClientAction = 'inserted' | 'updated' | 'removed' | 'changed';
export type EventSignatures = {
    'room.video-conference': (params: {
        rid: string;
        callId: string;
    }) => void;
    'shutdown': (params: Record<string, string[]>) => void;
    '$services.changed': (info: {
    ...
    'livechat-inquiry-queue-observer'(data: {
        action: string;
        inquiry: IInquiry;
    }): void;
    'message'(data: {
    ...
    'notify.updateInvites'(uid: string, data: {
        invite: Omit<IInvite, '_updatedAt'>;
    }): void;
    'notify.ephemeralMessage'(uid: string, rid: string, message: AtLeast<IMessage, 'msg'>): void;
    'notify.webdav'(uid: string, data: {
        type: 'changed';
        account: Partial<IWebdavAccount>;
    } | {
        type: 'removed';
        account: {
            _id: IWebdavAccount['_id'];
        };
    }): void;
    'notify.e2e.keyRequest'(rid: string, data: IRoom['e2eKeyId']): void;
    'notify.deleteMessage'(rid: string, data: {
        _id: string;
    }): void;
    'notify.deleteMessageBulk'(rid: string, data: {
        rid: string;
    ...
    'notify.deleteCustomSound'(data: {
        soundData: ICustomSound;
    }): void;
    'notify.updateCustomSound'(data: {
        soundData: ICustomSound;
    }): void;
    'notify.calendar'(uid: string, data: ICalendarNotification): void;
    'permission.changed'(data: {
    ...
    'watch.roles'(data: {
        clientAction: Exclude<ClientAction, 'removed'>;
        role: IRole;
    } | {
    ...
    'watch.rooms'(data: {
        clientAction: ClientAction;
        room: Pick<IRoom, '_id'> | IRoom;
    }): void;
    'watch.subscriptions'(data: {
        clientAction: 'updated' | 'inserted';
        subscription: Pick<ISubscription, 't' | 'ts' | 'ls' | 'lr' | 'name' | 'fname' | 'rid' | 'code' | 'f' | 'u' | 'open' | 'alert' | 'roles' | 'unread' | 'prid' | 'userMentions' | 'groupMentions' | 'archived' | 'audioNotificationValue' | 'desktopNotifications' | 'mobilePushNotifications' | 'emailNotifications' | 'desktopPrefOrigin' | 'mobilePrefOrigin' | 'emailPrefOrigin' | 'unreadAlert' | '_updatedAt' | 'blocked' | 'blocker' | 'autoTranslate' | 'autoTranslateLanguage' | 'disableNotifications' | 'hideUnreadStatus' | 'hideMentionStatus' | 'muteGroupMentions' | 'ignored' | 'E2EKey' | 'E2ESuggestedKey' | 'tunread' | 'tunreadGroup' | 'tunreadUser' | 'department' | 'v' | 'onHold'>;
    } | {
        clientAction: 'removed';
        subscription: {
            _id: string;
    ...
    'omnichannel.room'(roomId: string, data: {
        type: 'agentStatus';
        status: string;
    } | {
        type: 'queueData';
    ...
    'call.callerhangup'(userId: string, data: {
        roomId: string;
    }): void;
    'watch.pbxevents'(data: {
        clientAction: ClientAction;
        data: Partial<IPbxEvent>;
        id: string;
    }): void;
