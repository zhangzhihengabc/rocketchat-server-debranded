import type { IExportOperation, AvatarServiceObject, ISubscription, ITeam, IUser, IPersonalAccessToken, UserStatus } from '@rocket.chat/core-typings';
...
export declare const isUsersInfoProps: import("ajv").ValidateFunction<UsersInfo>;
type Users2faSendEmailCode = {
    emailOrUsername: string;
};
export declare const isUsers2faSendEmailCodeProps: import("ajv").ValidateFunction<Users2faSendEmailCode>;
type UsersSetAvatar = {
    userId?: IUser['_id'];
    username?: IUser['username'];
    avatarUrl?: string;
};
export declare const isUsersSetAvatarProps: import("ajv").ValidateFunction<UsersSetAvatar>;
type UsersResetAvatar = {
    userId?: IUser['_id'];
    username?: IUser['username'];
};
export declare const isUsersResetAvatarProps: import("ajv").ValidateFunction<UsersResetAvatar>;
type UsersPresencePayload = {
    users: UserPresence[];
    full: boolean;
};
export type UserPresence = Readonly<Partial<Pick<IUser, 'name' | 'status' | 'utcOffset' | 'statusText' | 'avatarETag' | 'roles' | 'username'>> & Required<Pick<IUser, '_id'>>>;
export type UserPersonalTokens = Pick<IPersonalAccessToken, 'name' | 'lastTokenPart' | 'bypassTwoFactor'> & {
    createdAt: string;
};
export type UsersEndpoints = {
    ...
    '/v1/users.listTeams': {
        GET: (params: UsersListTeamsParamsGET) => {
            teams: ITeam[];
        };
    };
