...
import type { FindOptions, FindCursor, UpdateResult, DeleteResult, Document, AggregateOptions, Filter, InsertOneResult, InsertManyResult } from 'mongodb';
...
export interface ISubscriptionsModel extends IBaseModel<ISubscription> {
    ...
    setAsReadByRoomIdAndUserId(rid: string, uid: string, readThreads?: boolean, alert?: boolean, options?: FindOptions<ISubscription>): ReturnType<IBaseModel<ISubscription>['update']>;
    ...
    findUsersInRoles<P extends Document = IUser>(roles: IRole['_id'][], rid: string | undefined, options: FindOptions<P extends IUser ? IUser : P>): Promise<FindCursor<P>>;
    findUsersInRoles<P extends Document = IUser>(roles: IRole['_id'][], rid: IRoom['_id'] | undefined, options?: FindOptions<P extends IUser ? IUser : P>): Promise<FindCursor<P>>;
    ...
    findByRolesAndRoomId({ roles, rid }: {
        roles: string;
        rid?: string;
    }, options?: FindOptions<ISubscription>): FindCursor<ISubscription>;
    ...
    findConnectedUsersExcept(userId: string, searchTerm: string, exceptions: string[], searchFields: string[], extraConditions: Filter<IUser>, limit: number, roomType?: ISubscription['t'], { startsWith, endsWith }?: {
        startsWith?: string | false;
        endsWith?: string | false;
    }, options?: AggregateOptions): Promise<SpotlightUser[]>;
    ...
    updateNotificationsPrefById(_id: string, notificationPref: {
        value: number;
        origin: string;
    } | null, notificationField: keyof ISubscription, notificationPrefOrigin: keyof ISubscription): Promise<UpdateResult>;
    ...
    findByUserIdExceptType(userId: string, typeException: ISubscription['t'], options?: FindOptions<ISubscription>): FindCursor<ISubscription>;
    ...
    incGroupMentionsAndUnreadForRoomIdExcludingUserId(roomId: string, userId: string, incGroup?: number, incUnread?: number): Promise<UpdateResult | Document>;
    ...
    incUserMentionsAndUnreadForRoomIdAndUserIds(roomId: string, userIds: string[], incUser?: number, incUnread?: number): Promise<UpdateResult | Document>;
    ignoreUser(data: {
        _id: string;
        ignoredUser: string;
        ignore?: boolean;
    ...
    updateNotificationUserPreferences(userId: string, userPref: string | number | boolean, notificationField: keyof ISubscription, notificationOriginField: keyof ISubscription): Promise<UpdateResult | Document>;
    clearNotificationUserPreferences(userId: string, notificationField: string, notificationOriginField: string): Promise<UpdateResult | Document>;
    ...
    createWithRoomAndManyUsers(room: IRoom, users: {
        user: AtLeast<IUser, '_id' | 'username' | 'name' | 'settings'>;
    ...
    addUnreadThreadByRoomIdAndUserIds(rid: string, users: string[], tmid: string, data: {
