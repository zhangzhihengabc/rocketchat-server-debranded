import type { IMessage, IRoom, IUser, ILivechatDepartment, MessageTypesValues, MessageAttachment, IMessageWithPendingFileImport } from '@rocket.chat/core-typings';
import type { AggregationCursor, CountDocumentsOptions, FindCursor, FindOptions, AggregateOptions, InsertOneResult, DeleteResult, UpdateResult, Document, Filter } from 'mongodb';
import type { FindPaginated, IBaseModel } from './IBaseModel';
export interface IMessagesModel extends IBaseModel<IMessage> {
    findPaginatedVisibleByMentionAndRoomId(username: IUser['username'], rid: IRoom['_id'], options?: FindOptions<IMessage>): FindPaginated<FindCursor<IMessage>>;
    ...
    findPaginatedByRoomIdAndType(roomId: IRoom['_id'], type: IMessage['t'], options?: FindOptions<IMessage>): FindPaginated<FindCursor<IMessage>>;
    ...
    getTotalOfMessagesSentByDate(params: {
        start: Date;
        end: Date;
        options?: any;
    }): Promise<any[]>;
    ...
    findVisibleByRoomIdNotContainingTypesAndUsers(roomId: IRoom['_id'], types: IMessage['t'][], users?: string[], options?: FindOptions<IMessage>, showThreadMessages?: boolean): FindCursor<IMessage>;
    findVisibleByRoomIdNotContainingTypesBeforeTs(roomId: IRoom['_id'], types: IMessage['t'][], ts: Date, options?: FindOptions<IMessage>, showThreadMessages?: boolean): FindCursor<IMessage>;
    ...
    createWithTypeRoomIdMessageUserAndUnread(type: MessageTypesValues, rid: string, message: string, user: Pick<IMessage['u'], '_id' | 'username' | 'name'>, unread?: boolean, extraData?: Partial<IMessage>): Promise<InsertOneResult<IMessage>>;
    ...
    setImportFileRocketChatAttachment(importFileId: string, rocketChatUrl: string, attachment: MessageAttachment): Promise<UpdateResult | Document>;
    ...
    findVisibleByRoomIdNotContainingTypes(roomId: string, types: MessageTypesValues[], options?: FindOptions<IMessage>, showThreadMessages?: boolean): FindCursor<IMessage>;
    findFilesByRoomIdPinnedTimestampAndUsers(rid: string, excludePinned: boolean, ignoreDiscussion: boolean, ts: Filter<IMessage>['ts'], users: string[], ignoreThreads: boolean, options?: FindOptions<IMessage>): FindCursor<IMessage>;
    findVisibleByRoomId<T extends IMessage = IMessage>(rid: string, options?: FindOptions<T>): FindCursor<T>;
    findDiscussionByRoomIdPinnedTimestampAndUsers(rid: string, excludePinned: boolean, ts: Filter<IMessage>['ts'], users: string[], options?: FindOptions<IMessage>): FindCursor<IMessage>;
    findVisibleByRoomIdAfterTimestamp(roomId: string, timestamp: Date, options?: FindOptions<IMessage>): FindCursor<IMessage>;
    findVisibleByRoomIdBeforeTimestampNotContainingTypes(roomId: string, timestamp: Date, types: MessageTypesValues[], options?: FindOptions<IMessage>, showThreadMessages?: boolean, inclusive?: boolean): FindCursor<IMessage>;
    findVisibleByRoomIdBetweenTimestampsNotContainingTypes(roomId: string, afterTimestamp: Date, beforeTimestamp: Date, types: MessageTypesValues[], options?: FindOptions<IMessage>, showThreadMessages?: boolean, inclusive?: boolean): FindCursor<IMessage>;
    countVisibleByRoomIdBetweenTimestampsNotContainingTypes(roomId: string, afterTimestamp: Date, beforeTimestamp: Date, types: MessageTypesValues[], showThreadMessages?: boolean, inclusive?: boolean): Promise<number>;
    ...
    setPinnedByIdAndUserId(_id: string, pinnedBy: Pick<IUser, '_id' | 'username'> | undefined, pinned?: boolean, pinnedAt?: Date): Promise<UpdateResult>;
    ...
    updateUsernameAndMessageOfMentionByIdAndOldUsername(_id: string, oldUsername: string, newUsername: string, newMessage: string): Promise<UpdateResult>;
    ...
    findThreadsByRoomIdPinnedTimestampAndUsers(data: {
        rid: string;
        pinned: boolean;
        ignoreDiscussion?: boolean;
        ts: Filter<IMessage>['ts'];
        users: string[];
    }, options?: FindOptions<IMessage>): FindCursor<IMessage>;
    removeByIdPinnedTimestampLimitAndUsers(rid: string, ignorePinned: boolean, ignoreDiscussion: boolean, ts: Filter<IMessage>['ts'], limit: number, users: string[], ignoreThreads: boolean, selectedMessageIds?: string[]): Promise<number>;
    findByIdPinnedTimestampLimitAndUsers(rid: string, ignorePinned: boolean, ignoreDiscussion: boolean, ts: Filter<IMessage>['ts'], limit: number, users: string[], ignoreThreads: boolean): Promise<string[]>;
    ...
    findUnreadThreadMessagesByDate(tmid: string, userId: string, after: Date): FindCursor<Pick<IMessage, '_id' | 't' | 'pinned' | 'drid' | 'tmid'>>;
