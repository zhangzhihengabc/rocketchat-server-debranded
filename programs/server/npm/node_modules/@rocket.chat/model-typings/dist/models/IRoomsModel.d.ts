...
export interface IRoomsModel extends IBaseModel<IRoom> {
    ...
    findByNameOrFnameContainingAndTypes(name: NonNullable<IRoom['name']>, types: Array<IRoom['t']>, discussion?: boolean, teams?: boolean, options?: FindOptions<IRoom>): FindPaginated<FindCursor<IRoom>>;
    findByTeamId(teamId: ITeam['_id'], options?: FindOptions<IRoom>): FindCursor<IRoom>;
    findPaginatedByTeamIdContainingNameAndDefault(teamId: ITeam['_id'], name: IRoom['name'], teamDefault: boolean, ids: Array<IRoom['_id']> | undefined, options?: FindOptions<IRoom>): FindPaginated<FindCursor<IRoom>>;
    ...
    findRoomsWithoutDiscussionsByRoomIds(name: NonNullable<IRoom['name']>, roomIds: Array<IRoom['_id']>, options?: FindOptions<IRoom>): FindCursor<IRoom>;
    findPaginatedRoomsWithoutDiscussionsByRoomIds(name: NonNullable<IRoom['name']>, roomIds: Array<IRoom['_id']>, options?: FindOptions<IRoom>): FindPaginated<FindCursor<IRoom>>;
    findChannelAndGroupListWithoutTeamsByNameStartingByOwner(name: NonNullable<IRoom['name']>, groupsToAccept: Array<IRoom['_id']>, options?: FindOptions<IRoom>): FindCursor<IRoom>;
    ...
    setTeamById(rid: IRoom['_id'], teamId: ITeam['_id'], teamDefault: NonNullable<IRoom['teamDefault']>, options?: UpdateOptions): Promise<UpdateResult>;
    ...
    allRoomSourcesCount(): AggregationCursor<{
        _id: Required<IOmnichannelGenericRoom['source']>;
        count: number;
    }>;
    ...
    findPaginatedByNameOrFNameAndRoomIdsIncludingTeamRooms(searchTerm: RegExp | null, teamIds: Array<ITeam['_id']>, roomIds: Array<IRoom['_id']>, options?: FindOptions<IRoom>): FindPaginated<FindCursor<IRoom>>;
    findPaginatedContainingNameOrFNameInIdsAsTeamMain(searchTerm: RegExp | null, rids: Array<IRoom['_id']>, options?: FindOptions<IRoom>): FindPaginated<FindCursor<IRoom>>;
    ...
    setDmReadOnlyByUserId(roomId: string, ids: string[] | undefined, readOnly: NonNullable<IRoom['ro']>, reactWhenReadOnly: NonNullable<IRoom['reactWhenReadOnly']>): Promise<UpdateResult | Document>;
    getDirectConversationsByUserId(userId: string, options?: FindOptions<IRoom>): FindCursor<IRoom>;
    setAllowReactingWhenReadOnlyById(roomId: string, allowReactingWhenReadOnly: NonNullable<IRoom['reactWhenReadOnly']>): Promise<UpdateResult>;
    ...
    findOneByNameAndType(name: string, type: IRoom['t'], options?: FindOptions<IRoom>, includeFederatedRooms?: boolean): Promise<IRoom | null>;
    ...
    findByNameAndTypeNotDefault(name: IRoom['name'] | RegExp, type: IRoom['t'], options?: FindOptions<IRoom>, includeFederatedRooms?: boolean): FindCursor<IRoom>;
    findByNameOrFNameAndTypesNotInIds(name: IRoom['name'] | RegExp, types: IRoom['t'][], ids: string[], options?: FindOptions<IRoom>, includeFederatedRooms?: boolean): FindCursor<IRoom>;
    ...
    setAnnouncementById(rid: string, announcement: IRoom['announcement'], announcementDetails: IRoom['announcementDetails']): Promise<UpdateResult>;
