...
export interface IFederationRoomEventsModel extends IBaseModel<IFederationEvent> {
    ...
    createAddUserEvent(origin: string, roomId: string, user: IUser, subscription: ISubscription, domainsAfterAdd: string[]): Promise<Omit<IFederationEvent, '_updatedAt'>>;
    createRemoveUserEvent(origin: string, roomId: string, user: IUser, domainsAfterRemoval: string[]): Promise<Omit<IFederationEvent, '_updatedAt'>>;
    createUserLeftEvent(origin: string, roomId: string, user: IUser, domainsAfterLeave: string[]): Promise<Omit<IFederationEvent, '_updatedAt'>>;
    createMessageEvent(origin: string, roomId: string, message: string): Promise<Omit<IFederationEvent, '_updatedAt'>>;
    createEditMessageEvent(origin: string, roomId: string, originalMessage: {
        _id: string;
        msg: string;
    ...
    createSetMessageReactionEvent(origin: string, roomId: string, messageId: string, username: string, reaction: string): Promise<Omit<IFederationEvent, '_updatedAt'>>;
    createUnsetMessageReactionEvent(origin: string, roomId: string, messageId: string, username: string, reaction: string): Promise<Omit<IFederationEvent, '_updatedAt'>>;
