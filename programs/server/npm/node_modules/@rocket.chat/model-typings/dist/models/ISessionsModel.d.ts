...
export interface ISessionsModel extends IBaseModel<ISession> {
    aggregateSessionsAndPopulate({ sort, search, offset, count, }: {
        sort?: Record<CustomSortOpAdmin, 1 | -1>;
        search?: string | null;
        offset?: number;
        count?: number;
    }): Promise<{
        sessions: Array<DeviceManagementPopulatedSession>;
        count: number;
        offset: number;
        total: number;
    }>;
    aggregateSessionsByUserId({ uid, sort, search, offset, count, }: {
        uid: string;
        ...
        sessions: Array<DeviceManagementSession>;
        count: number;
        offset: number;
        total: number;
    }>;
    ...
    getBusiestTimeWithinHoursPeriod({ start, end, groupSize }: DateRange & {
        groupSize: number;
    }): Promise<{
        hour: number;
        users: number;
    ...
    updateActiveSessionsByDateAndInstanceIdAndIds(params: Partial<DestructuredDate>, instanceId: string, sessions: string[], data: Record<string, any>): Promise<UpdateResult | Document>;
    ...
    logoutBySessionIdAndUserId({ sessionId, userId, }: {
        sessionId: ISession['sessionId'];
        userId: IUser['_id'];
    }): Promise<UpdateResult | Document>;
    logoutByloginTokenAndUserId({ loginToken, userId, logoutBy, }: {
