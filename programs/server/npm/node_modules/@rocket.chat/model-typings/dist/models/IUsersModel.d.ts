import type { IUser, IRole, IRoom, ILivechatAgent, UserStatus, ILoginToken, IPersonalAccessToken, AtLeast, ILivechatAgentStatus } from '@rocket.chat/core-typings';
...
export interface IUsersModel extends IBaseModel<IUser> {
    ...
    findPaginatedUsersInRolesWithQuery<T = IUser>(roles: IRole['_id'] | IRole['_id'][], query: any, options: any): FindPaginated<FindCursor<T>>;
    ...
    findByActiveUsersExcept<T = IUser>(searchTerm: any, exceptions: any, options: any, searchFields: any, extraQuery?: any, params?: {
        startsWith?: boolean;
        endsWith?: boolean;
    }): FindCursor<T>;
    findPaginatedByActiveUsersExcept<T = IUser>(searchTerm: any, exceptions: any, options: any, searchFields: any, extraQuery?: any, params?: {
        startsWith?: boolean;
        endsWith?: boolean;
    }): FindPaginated<FindCursor<T>>;
    findPaginatedByActiveLocalUsersExcept<T = IUser>(searchTerm: any, exceptions: any, options: any, forcedSearchFields: any, localDomain: any): FindPaginated<FindCursor<T>>;
    findPaginatedByActiveExternalUsersExcept<T = IUser>(searchTerm: any, exceptions: any, options: any, forcedSearchFields: any, localDomain: any): FindPaginated<FindCursor<T>>;
    ...
    getNextLeastBusyAgent(department: any, ignoreAgentId: any): Promise<{
        agentId: string;
        username: string;
    ...
    getLastAvailableAgentRouted(department: any, ignoreAgentId: any): Promise<{
        agentId: string;
        username: string;
        lastRoutingTime: Date;
        departments: any[];
    ...
    getAgentAndAmountOngoingChats(userId: any): Promise<{
        agentId: string;
        username: string;
        lastAssignTime: Date;
        lastRoutingTime: Date;
    ...
    findActiveByUsernameOrNameRegexWithExceptionsAndConditions<T = IUser>(termRegex: any, exceptions: any, conditions: any, options: any): FindCursor<T>;
    countAllAgentsStatus({ departmentId }: {
        departmentId?: any;
    }): any;
    ...
    updateStatusById(userId: string, { statusDefault, status, statusConnection, statusText, }: {
    ...
    getUnavailableAgents(departmentId?: string, extraQuery?: Document): Promise<{
        agentId: string;
        username: string;
    ...
    findOneOnlineAgentByUserList(userList: string[] | string, options?: FindOptions<IUser>, isLivechatEnabledWhenAgentIdle?: boolean): Promise<IUser | null>;
    ...
    addPersonalAccessTokenToUser(data: {
        userId: string;
        loginTokenObject: IPersonalAccessToken;
    }): Promise<UpdateResult>;
    removePersonalAccessTokenOfUser(data: {
        userId: string;
        loginTokenObject: AtLeast<IPersonalAccessToken, 'type' | 'name'>;
    }): Promise<UpdateResult>;
    findPersonalAccessTokenByTokenNameAndUserId(data: {
        userId: string;
        tokenName: string;
    ...
    getNextAgent(ignoreAgentId?: string, extraQuery?: Filter<IUser>): Promise<{
        agentId: string;
        username: string;
    } | null>;
    getNextBotAgent(ignoreAgentId?: string): Promise<{
        agentId: string;
        username: string;
    } | null>;
    ...
    getAgentInfo(agentId: string, showAgentEmail?: boolean): Promise<Pick<ILivechatAgent, '_id' | 'name' | 'username' | 'phone' | 'customFields' | 'status' | 'livechat'> | null>;
    roleBaseQuery(userId: string): {
        _id: string;
    };
    setE2EPublicAndPrivateKeysByUserId(userId: string, e2e: {
        public_key: string;
        private_key: string;
    }): Promise<UpdateResult>;
    rocketMailUnsubscribe(userId: string, createdAt: string): Promise<number>;
    fetchKeysByUserId(userId: string): Promise<{
        public_key: string;
        private_key: string;
    } | Record<string, never>>;
    ...
    findOneByUsernameAndServiceNameIgnoringCase(username: string, userId: string, serviceName: string, options?: FindOptions<IUser>): Promise<IUser | null>;
    findOneByEmailAddressAndServiceNameIgnoringCase(emailAddress: string, userId: string, serviceName: string, options?: FindOptions<IUser>): Promise<IUser | null>;
    ...
    resetPasswordAndSetRequirePasswordChange(userId: string, requirePasswordChange: boolean, requirePasswordChangeReason: string): Promise<UpdateResult>;
    ...
    findAgentsWithDepartments<T = ILivechatAgent>(role: string, query: Filter<IUser>, options: FindOptions<IUser>): Promise<{
        sortedResults: (T & {
            departments: string[];
        })[];
        totalCount: {
