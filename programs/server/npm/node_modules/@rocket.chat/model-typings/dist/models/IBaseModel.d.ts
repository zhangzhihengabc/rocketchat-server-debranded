...
import type { BulkWriteOptions, ChangeStream, Collection, DeleteOptions, DeleteResult, Document, EnhancedOmit, Filter, FindCursor, FindOneAndUpdateOptions, FindOptions, InsertManyResult, InsertOneOptions, InsertOneResult, ModifyResult, OptionalId, UpdateFilter, UpdateOptions, UpdateResult, WithId } from 'mongodb';
export type DefaultFields<Base> = Record<keyof Base, 1> | Record<keyof Base, 0> | void;
export type ResultFields<Base, Defaults> = Defaults extends void ? Base : Defaults[keyof Defaults] extends 1 ? Pick<Defaults, keyof Defaults> : Omit<Defaults, keyof Defaults>;
...
export interface IBaseModel<T extends {
    _id: string;
}, C extends DefaultFields<T> = undefined, TDeleted extends RocketChatRecordDeleted<T> = RocketChatRecordDeleted<T>> {
    col: Collection<T>;
    ...
    find<P extends Document>(query: Filter<T> | undefined, options?: FindOptions<P extends T ? T : P>): FindCursor<WithId<P>> | FindCursor<WithId<T>>;
    ...
    update(filter: Filter<T>, update: UpdateFilter<T> | Partial<T>, options?: UpdateOptions & {
        multi?: true;
    ...
    trashFind<P extends TDeleted>(query: Filter<TDeleted>, options?: FindOptions<P extends TDeleted ? TDeleted : P>): FindCursor<WithId<TDeleted>> | undefined;
    ...
    trashFindOneById<P extends TDeleted>(_id: TDeleted['_id'], options?: FindOptions<P extends TDeleted ? TDeleted : P>): Promise<WithId<RocketChatRecordDeleted<P> | TDeleted> | null>;
    trashFindDeletedAfter(deletedAt: Date): FindCursor<WithId<TDeleted>>;
    trashFindDeletedAfter<P extends Document = TDeleted>(deletedAt: Date, query?: Filter<TDeleted>, options?: FindOptions<P extends TDeleted ? TDeleted : P>): FindCursor<WithId<TDeleted>>;
    trashFindPaginatedDeletedAfter<P extends Document = TDeleted>(deletedAt: Date, query?: Filter<TDeleted>, options?: FindOptions<P extends TDeleted ? TDeleted : P>): FindPaginated<FindCursor<WithId<TDeleted>>>;
