...
class Appservice extends events_1.EventEmitter {
    ...
    begin() {
        return new Promise((resolve, reject) => {
            ...
            var _a;
            if ((_a = this.options.intentOptions) === null || _a === void 0 ? void 0 : _a.encryption) {
                yield this.botIntent.enableEncryption();
            }
            else {
                yield this.botIntent.ensureRegistered();
            }
        }));
    }
    /**
    ...
    isNamespacedUser(userId) {
        ...
        return userId === this.botUserId ||
            !!((_a = this.registration.namespaces) === null || _a === void 0 ? void 0 : _a.users.find(({ regex }) => new RegExp(regex).test(userId)));
    }
    /**
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.pendingTransactions[txnId] = new Promise((resolve) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (Array.isArray(req.body["de.sorunome.msc2409.to_device"])) {
                    orderedEdus.push(...req.body["de.sorunome.msc2409.to_device"].map(e => (Object.assign(Object.assign({}, e), { unsigned: Object.assign(Object.assign({}, e['unsigned']), { [EDU_ANNOTATION_KEY]: EduAnnotation.ToDevice }) }))));
                }
                if (Array.isArray(req.body["de.sorunome.msc2409.ephemeral"])) {
                    orderedEdus.push(...req.body["de.sorunome.msc2409.ephemeral"].map(e => (Object.assign(Object.assign({}, e), { unsigned: Object.assign(Object.assign({}, e['unsigned']), { [EDU_ANNOTATION_KEY]: EduAnnotation.Ephemeral }) }))));
                }
                ...
                if (this.cryptoStorage) {
                    const deviceLists = (_a = req.body["org.matrix.msc3202.device_lists"]) !== null && _a !== void 0 ? _a : { changed: [], removed: [] };
                    ...
                    if (otks) {
                        for (const userId of Object.keys(otks)) {
                            ...
                            if (otksForUser) {
                                if (!byUserId[userId])
                                    byUserId[userId] = { counts: null, toDevice: null, unusedFallbacks: null };
                    ...
                    for (const userId of Object.keys(byUserId)) {
                        ...
                        if (!info.unusedFallbacks)
                            info.unusedFallbacks = JSON.parse((yield userStorage.readValue("last_unused_fallbacks")) || "[]");
                        if (!info.counts)
                            info.counts = JSON.parse((yield userStorage.readValue("last_counts")) || "{}");
                        __1.LogService.info("Appservice", `Updating crypto state for ${userId}`);
                        yield intent.underlyingClient.crypto.updateSyncData(info.toDevice, info.counts, info.unusedFallbacks, deviceLists.changed, deviceLists.removed);
                    }
                }
                for (let event of req.body["events"]) {
                    ...
                    if (event['type'] === 'm.room.encrypted' && this.cryptoStorage) {
                        ...
                        try {
                            ...
                            try {
                                event = (yield this.botClient.crypto.decryptRoomEvent(encrypted, roomId)).raw;
                                event = yield this.processEvent(event);
                                ...
                                try {
                                    ...
                                    const userIdsInRoom = yield this.botClient.getJoinedRoomMembers(roomId);
                                ...
                                if (tryUserId) {
                                    ...
                                    event = (yield intent.underlyingClient.crypto.decryptRoomEvent(encrypted, roomId)).raw;
                                    event = yield this.processEvent(event);
                                    this.emit("room.decrypted_event", roomId, event);
                                    // For logging purposes: show that the event was decrypted
                                    __1.LogService.info("Appservice", `Processing decrypted event of type ${event["type"]}`);
