"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.encrypt = exports.decrypt = void 0;
const crypto_1 = __importDefault(require("crypto"));
const jwt_1 = require("@rocket.chat/jwt");
const PUBLIC_LICENSE_KEY_V2 = '';
const PUBLIC_LICENSE_KEY_V3 = Buffer.from(PUBLIC_LICENSE_KEY_V2, 'base64').toString('utf-8');
let TEST_KEYS = undefined;
async function decrypt(encrypted) {
    if (process.env.NODE_ENV === 'test') {
        if (encrypted.startsWith('RCV3_')) {
            const jwt = encrypted.substring(5);
            TEST_KEYS = TEST_KEYS ?? (await (0, jwt_1.getPairs)());
            if (!TEST_KEYS) {
                throw new Error('Missing PUBLIC_LICENSE_KEY_V3');
            }
            const [spki] = TEST_KEYS;
            const [payload] = await (0, jwt_1.verify)(jwt, spki);
            return JSON.stringify(payload);
        }
    }
    // handle V3
    if (encrypted.startsWith('RCV3_')) {
        const jwt = encrypted.substring(5);
        const [payload] = await (0, jwt_1.verify)(jwt, PUBLIC_LICENSE_KEY_V3);
        return JSON.stringify(payload);
    }
    const decrypted = crypto_1.default.publicDecrypt(Buffer.from(PUBLIC_LICENSE_KEY_V2, 'base64').toString('utf-8'), Buffer.from(encrypted, 'base64'));
    return decrypted.toString('utf-8');
}
exports.decrypt = decrypt;
async function encrypt(license) {
    if (process.env.NODE_ENV !== 'test') {
        throw new Error('This function should only be used in tests');
    }
    TEST_KEYS = TEST_KEYS ?? (await (0, jwt_1.getPairs)());
    const [, pkcs8] = TEST_KEYS;
    return `RCV3_${await (0, jwt_1.sign)(license, pkcs8)}`;
}
exports.encrypt = encrypt;
//# sourceMappingURL=token.js.map