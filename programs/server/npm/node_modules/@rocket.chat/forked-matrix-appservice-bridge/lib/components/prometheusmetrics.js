 ...
 * @constructor
 */
const log = logging_1.default.get('PrometheusMetrics');
    ...
    registerMatrixSdkMetrics(appservice) {
        ...
        appservice.metrics.registerListener({
            ...
            onIncrement: (metricName, context) => {
                if (metricName === forked_matrix_bot_sdk_1.METRIC_MATRIX_CLIENT_SUCCESSFUL_FUNCTION_CALL) {
                    const ctx = context;
                    callCounts.inc({ method: ctx.functionName });
                }
                if (metricName === forked_matrix_bot_sdk_1.METRIC_MATRIX_CLIENT_FAILED_FUNCTION_CALL) {
                    const ctx = context;
...
PrometheusMetrics.AgeCounters = agecounters_1.AgeCounters;
